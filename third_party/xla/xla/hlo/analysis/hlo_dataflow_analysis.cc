...
bool HloDataflowAnalysis::UpdateAsyncChainOutputValueSet(
  ...
  CHECK(async_shape.IsTuple() && async_shape.tuple_shapes().size() >= 2);
  const Shape& output_shape = async_op->shape().tuple_shapes(1);

  if (output_shape.IsTuple() && output_shape.tuple_shapes().empty()) {
    // The output is not bound yet.
    return changed;
  }
  ...
  ShapeUtil::ForEachSubshape(
      root->shape(), [&](const Shape& subshape, const ShapeIndex& index) {
        ...
        if (!ShapeUtil::IndexIsValid(async_shape, output_index)) {
          return;
        }
